...
After I could not make yanto's code work reliably on my computer (for wathever reason I can never make other people's work on my machine), I built my own computer-vision pipeline and calibration flow. The CV side in this repo is custom and designed to be easily recalibrated for other setups. The solver, however, is stright up taken by Yanto's work.
...
In short: spatial calibration + edge/mask features + multi-metric shape comparison + temporal differencing.
...
5. Red/black classification from HSV cues for numbered cards
